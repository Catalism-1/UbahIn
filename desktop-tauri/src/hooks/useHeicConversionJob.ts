...
} from '../pages/HeicToImagePage/types';
import type { JobStatus } from '../pages/ImageToPdfPage/types';

const MAX_FILES = 50;

export const DEFAULT_HEIC_JOB_DEFAULTS: HeicToImageOptions = {
  outputDirectory: '',
  outputFormat: 'jpg',
...
export function useHeicConversionJob(isEngineReady: boolean, defaults: HeicToImageOptions = DEFAULT_HEIC_JOB_DEFAULTS) {
  const [files, setFiles] = useState<HeicQueueItem[]>([]);
  const [options, setOptions] = useState<HeicToImageOptions>(defaults);
  const [status, setStatus] = useState<JobStatus>('idle');
  const [activeJobId, setActiveJobId] = useState<string | null>(null);
  const [progress, setProgress] = useState<HeicConversionProgress | null>(null);
  const [result, setResult] = useState<HeicConversionResult | null>(null);
  ...
  const handlePickFiles = useCallback(async () => {
    if (isBusy) return;
    try {
      const pickedPaths = await pickHeicFiles();
      ...
      if (allowedPaths.length === 0) {
        addToast('Tidak ada foto baru ditambahkan.', 'warning', 'File duplikat atau antrean sudah mencapai batas 50 foto.');
        return;
      }

      setStatus('inspecting');
      const inspected = await inspectHeicFiles(allowedPaths);
      const queueItems = inspected.map((file): HeicQueueItem => {
      ...
      addToast('Gagal memeriksa foto HEIC.', 'error', error instanceof Error ? error.message : 'Silakan coba lagi.');
      console.error(error);
    }
  ...
  const startJob = useCallback(async () => {
    ...
    try {
      await startHeicConversion(jobId, validFiles, options);
  ...
  const cancelJob = useCallback(async () => {
    ...
    try {
      await cancelHeicConversionJob(activeJobId);
    } catch (error) {
  ...
  const handleProgressEvent = useCallback((event: HeicConversionProgress) => {
  ...
  const finishJob = useCallback((event: HeicConversionResult, nextStatus: JobStatus) => {
    const currentJobId = activeJobIdRef.current;
    if (event.job_id && currentJobId && event.job_id !== currentJobId) return;
    ...
    if (nextStatus === 'cancelled') {
      ...
      addToast('Konversi gagal.', 'error', event.errors[0] ?? 'Gagal mengonversi file HEIC. Silakan buka log.');
    } else if (event.failed_files > 0 && event.successful_files > 0) {
      addToast('Selesai dengan beberapa file gagal.', 'warning', 'Beberapa foto HEIC tidak dapat dikonversi.');
    } else {
      addToast('Konversi selesai.', 'success');
    }
  }, [addToast]);

  useTauriEvent<HeicConversionProgress>('engine://progress', handleProgressEvent, true);
  ...
  useTauriEvent<HeicConversionResult>('engine://job-completed', (event) => finishJob(event, 'completed'), true);
  useTauriEvent<HeicConversionResult>('engine://job-failed', (event) => finishJob(event, 'failed'), true);
